  ...
  <div class="categorias-container" v-if="categorias">
    ...
    <div v-for="(categoria, key) in categorias.genres" :key="key">
      <transition mode="out-in">
        <div
          :class="{ desativo: letter && categoria.name.slice(0, 1) !== letter }"
        >
          <div class="categorias-flex">
            ...
            <router-link
              :to="{
                name: 'categoria',
                params: { id: categoria.id, nome: categoria.name },
                query: { page: 1 },
              }"
            >
...
<script>
import CategoriasLetras from "@/components/CategoriaLetras.vue";
import FilmesDaCategoria from "@/components/filmes/FilmesDaCategoria.vue";
...
export default {
  ...
  computed: {
    ...
    letrasInicias() {
      const arrayLetras = this.categorias.genres.map((categoria) =>
        categoria.name.slice(0, 1)
      );
