<template>
  <transition mode="out-in" v-if="carousel && listaFilmes">
    <carousel
      :responsive="{0:{items: 1}, 300:{items: 2}, 600:{items:3}, 1000:{items: 4}}" 
      :nav="false"
      :dots="false" 
      :margin="20"
      ...
      <div class="filme" v-for="(filme, index) in listaFilmes.results" :key="index" :class="{ ativo: filme.poster_path  }">
        <router-link :to="{ name: 'filmes', params: { id: filme.id } }">
          <b class="filme-titulo">{{ filme.title }}</b>
          <img v-if="filme.poster_path" :src="filme.poster_path | filmeImagem" :alt="filme.title">
    ...
    <div class="filmes-container" v-else-if="!carousel && listaFilmes">
      <div class="filme" v-for="(filme, index) in listaFilmes.results" :key="index" :class="{ ativo: filme.backdrop_path  }">
        <router-link :to="{ name: 'filmes', params: { id: filme.id } }">
          <b class="filme-titulo">{{ filme.title }}</b>
...
<style>
  ...
  .prev {
    background: rgba(0, 0, 0, .7) url('../assets/prev.svg') no-repeat center center;
  }

  .next {
    background: rgba(0, 0, 0, .7) url('../assets/next.svg') no-repeat center center;
  }
