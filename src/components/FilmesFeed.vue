      ...
      <div v-if="filmesBuscados && filmesBuscados.results.length > 0">
        ...
        <transition mode="out-in">
          <div class="filmes-container" v-if="filmesBuscados">
            <div class="filme" v-for="(filme, index) in filmesBuscados.results" :key="index" :class="{ ativo: filme.backdrop_path  }">
        ...
        <div class="paginacao">
          <FilmesPaginacao :totalFilmes="filmesBuscados.total_pages" :filmesPorPagina="20" />
        </div>
      </div>
      <div v-else>
