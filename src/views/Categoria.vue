    ...
    <transition mode="out-in">
      <div v-if="filmesDaCategoria">
        <div class="filmes-container" v-if="filmesDaCategoria">
          <div class="filme" v-for="(filme, index) in filmesDaCategoria.results" :key="index" :class="{ ativo: filme.backdrop_path  }">
        ...
        <div class="paginacao">
          <FilmesPaginacao :totalFilmes="filmesDaCategoria.total_pages" :filmesPorPagina="20" />
        </div>
      </div>
      <PageLoading v-else />
    </transition>
    ...
    methods: {
      async puxarFilmesECategorias() {
        ...
        try {
          ...
          tmdbApi
            .get(`/discover/movie?page=${this.pagina ? this.pagina : 1}
      ...
      mudarRota({ target }) {
        ...
        this.checkCurrentRouteAndRedirect(this.$router, `/categorias/${id}?page=1`);
        this.idFilme = id;
